...
# x = requests.get(url)
#ip: 10.0.0.146
#username: 14rsqBh25OQBcnpBuFEWfvmYsIryXcyuKcNWu3XW
...
def initialize():
    ...
    if len(sys.argv) >= 3:
        ...
        LIGHT_ARRAY = int(sys.argv[1])
        HUE_IP = sys.argv[2]
        HUE_APIKEY = sys.argv[3]
        print("LIGHT_ARRAY:", LIGHT_ARRAY)
        print("HUE_IP:", HUE_IP)
        print("HUE_APIKEY: ", HUE_APIKEY)
    else:
        # Ask Questions from the User
        LIGHT_ARRAY = int(input("Light ARRAY: "))
        HUE_IP = input("IP Address: ")
        HUE_APIKEY = input("Hue API Key: ")
